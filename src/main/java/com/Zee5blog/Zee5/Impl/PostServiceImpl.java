...
public class PostServiceImpl implements PostService {
	...
	public PostDto createPost(PostDto postDto, Integer userId, Integer categoryId) {
		User user = this.userRepository.findById(userId)
				.orElseThrow(() -> new ResourceNotFountException("User", "User id", userId));
	...
	public PostDto updatePost(PostDto postDto, Integer postId) {
		Post post = this.postRepository.findById(postId)
				.orElseThrow(() -> new ResourceNotFountException("Post", "Post Id", postId));
	...
	public PostDto getPostById(Integer postId) {
		Post post = this.postRepository.findById(postId)
				.orElseThrow(() -> new ResourceNotFountException("User", "User id", postId));
	...
	/*
	 * @Override public List<PostDto> searchPosts(String keyword) { List<Post>
	 * posts=this.postRepository.searchByTitleContaining(keyword); List<PostDto>
	 * postSearching=posts.stream().map((post)->this.modelMapper.map(posts,
	 * PostDto.class)).collect(Collectors.toList()); return postSearching; }
	 */
	...
	@Override
	public List<PostDto> searchByTitleContaining(String keywords) {
		List<Post> posts=this.postRepository.searchByTitleContaining(keywords);
		List<PostDto> postSearching=posts.stream().map((post)->this.modelMapper.map(post, PostDto.class)).collect(Collectors.toList());
		return postSearching;
	}
